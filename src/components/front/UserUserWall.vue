<template>
  <div class="col-8">
    <div v-if="targetUserId === profile._id" class="row">
      <div class="col shadow-new-post">
        <div class="bg-white border border-2 border-black azeret-mono fs-5 fw-bold py-5 mb-6 w-100 text-center">
        我的個人牆
        </div>
      </div>
    </div>
    <div v-else class="row mb-6">
      <div class="col shadow-user-wall">
        <div v-if="userPosts.targetUser" class="d-flex bg-white rounded border border-2 border-black">
          <div class="overflow-hidden rounded-start" style="width: 80px; height: 80px;">
            <img :src="userPosts.targetUser.photo" :alt="`user-photo-${userPosts.targetUser.name}`" class="object-fit-cover img-fluid" style="height: 80px;">
          </div>
    ...
    <div class="row mb-4">
      <div class="col-4">
        <select @input="getUserPosts(targetUserId, timeSort)" v-model="timeSort" class="form-select rounded-0 border-2 border-black py-3 ps-4 pe-10" aria-label="posts-select">
    ...
    <!-- 動態牆-無動態 -->
    <div v-if="!userPosts.posts" class="row">
      <div class="col">
    ...
    <!-- 動態牆-有動態 -->
    <div v-if="userPosts.posts" class="row gy-6">
        ...
        <div class="bg-white rounded border border-2 border-black p-6" style="border-bottom: 4px solid black !important">
          ...
          <div class="d-flex align-items-center mb-5">
            <div v-if="post.likes.length" class="d-flex align-items-center gap-2">
              ...
              <small v-if="post.likes[0]" class="noto-sans-tc">{{ post.likes[0].user.name }} 和其他 {{ post.likes.length - 1 }}人按讚</small>
            </div>
            <div class="ms-auto">
              <div class="d-flex align-items-center gap-2">
                <button @click="handleLikePost('userwall', post._id, profile._id, targetUserId)" type="button" class="btn p-0">
